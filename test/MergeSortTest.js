...
describe('MergeSort', () => {
  it("merge sort should throw an error when the custom comparator doesn't return -1, 1, or 0", () => {
    ...
    // No error
    mergeSort(arr, zeroComp);
    mergeSort(arr, negativeComp);
    mergeSort(arr, positiveComp);

    // Error
    assert.throws(() => { mergeSort(arr, wrongNegativeComp); }, Error);
    assert.throws(() => { mergeSort(arr, wrongPositiveComp); }, Error);
  });

  it("merge sort sort should, with a comparator, sort an array in a defined order", () => {
  ...
  it('merge sort should, without a comparator, sort an array in ascending order', () => {
